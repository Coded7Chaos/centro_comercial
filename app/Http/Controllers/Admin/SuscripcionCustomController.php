...
class SuscripcionCustomController extends Controller
{
    ...
    public function create()
    {
        ...
        // Load all shops and determine availability and days until release
        $tiendasRaw = InfraestructurasTiendas::with(['piso.infraestructura'])->get();
        $tiendasData = [];
    ...
    public function store(Request $request)
    {
        ...
        $fecha_fin = $fecha_inicio->copy()->addMonths($totalMonths)->subDay()->toDateString();
    ...
    public function descargarContrato($id)
    {
        ...
        if (request('format') === 'word') {
            ...
            $fontStyle = array('name' => 'Arial', 'size' => 11, 'color' => '1F2937');
            $boldFont = array('name' => 'Arial', 'size' => 11, 'bold' => true, 'color' => '1F2937');
            $underlineBoldFont = array('name' => 'Arial', 'size' => 11, 'bold' => true, 'underline' => 'single', 'color' => '1F2937');

            $titleFont = array('name' => 'Arial', 'size' => 18, 'bold' => true, 'color' => '111827');
            $subtitleFont = array('name' => 'Arial', 'size' => 11, 'color' => '4B5563');
            $sectionTitleFont = array('name' => 'Arial', 'size' => 13, 'bold' => true, 'color' => '111827');

            $paraStyle = array('alignment' => 'justify', 'spaceAfter' => 120, 'lineHeight' => 1.15);
            $centerParaStyle = array('alignment' => 'center', 'spaceAfter' => 120, 'lineHeight' => 1.15);
            $leftParaStyle = array('alignment' => 'left', 'spaceAfter' => 120, 'lineHeight' => 1.15);
            ...
            $thStyle = array('bgColor' => 'F3F4F6');
            $thFont = array('name' => 'Arial', 'size' => 10, 'bold' => true, 'color' => '111827');
            $tdFont = array('name' => 'Arial', 'size' => 10, 'color' => '1F2937');
            $tdFontBold = array('name' => 'Arial', 'size' => 10, 'bold' => true, 'color' => '1F2937');
            ...
            $table->addCell(2500)->addText(\Carbon\Carbon::parse($suscripcion->fecha_inicio)->format('d/m/Y'), $tdFont);
            ...
            $textRun5->addText(\Carbon\Carbon::parse($suscripcion->fecha_inicio)->format('d/m/Y'), $boldFont);
            $textRun5->addText(" hasta el ", $fontStyle);
            $textRun5->addText(\Carbon\Carbon::parse($suscripcion->fecha_fin)->format('d/m/Y'), $boldFont);
            ...
            // Signatures
            $sigTable = $section->addTable(array('borderSize' => 0, 'cellMargin' => 100));
            $sigTable->addRow();
            $cellLeft = $sigTable->addCell(5000);
            $cellLeft->addText("_______________________________", $fontStyle, array('alignment' => 'center'));
            $cellLeft->addText("EL ARRENDADOR", $boldFont, array('alignment' => 'center'));
            $cellLeft->addText("Administración " . ($infraestructura?->nombre ?? 'Mall'), $fontStyle, array('alignment' => 'center'));

            $cellRight = $sigTable->addCell(5000);
            $cellRight->addText("_______________________________", $fontStyle, array('alignment' => 'center'));
            $cellRight->addText("EL ARRENDATARIO", $boldFont, array('alignment' => 'center'));
            $cellRight->addText("CI: " . $cliente?->ci, $fontStyle, array('alignment' => 'center'));

            // Footer
            $section->addTextBreak(2);
            $section->addText("Documento generado automáticamente por el sistema de control comercial del " . ($infraestructura?->nombre ?? 'Mall') . ".", array('name' => 'Arial', 'size' => 9, 'italic' => true, 'color' => '9CA3AF'), $centerParaStyle);
    ...
    public function storePayment(Request $request, $id)
    {
        ...
        if ($saldoPendiente > 0) {
            // Defer outstanding balance to next month
            $fechaVencimientoCobro = Carbon::parse($cobro->fecha_vencimiento)->addMonth()->toDateString();
